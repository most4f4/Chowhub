        ...
        <Row>
          {teamMembers.map((m) => (
            <Col key={m.name} md={6} lg={3} className="mb-4">
              ...
              >
                <div
                  style={{ position: "relative", width: "100%", height: "0", paddingTop: "100%" }}
                >
                  <Image src={m.image} alt={m.name} fill style={{ objectFit: "cover" }} />
                </div>
                <Card.Body className="d-flex flex-column">
                  <Card.Title style={{ color: m.color, fontSize: "1.25rem" }}>{m.name}</Card.Title>
                  <Card.Subtitle className="mb-3 text-secondary">{m.role}</Card.Subtitle>
